        ...
        <div className="flex-1 px-5 py-5">
          ...
          <ul className="mt-5 divide-y divide-line border-y border-line">
            {habits.map((habit, index) => (
              <li key={habit.id} className="flex items-center gap-2 py-2.5">
                {editingId === habit.id ? (
                  <input
                    autoFocus
                    className={inputClass}
          ...
          {confirmingId && (
            <div className="anim-rise mt-5 rounded-sm border border-[#e8cfca] bg-[#fdf3f1] px-4 py-3.5">
              <p className="text-[14.5px] text-ink">
                Delete “{habits.find((h) => h.id === confirmingId)?.name}”? Its entries, notes, and
                photos go with it.
              </p>
...
function IconButton({
  ...
  return (
    <button
      ...
      disabled={disabled}
      onClick={onClick}
      className={`flex h-8 w-8 items-center justify-center rounded-sm transition-colors duration-150 disabled:opacity-30 ${
        danger ? "text-muted hover:bg-[#f7ebe8] hover:text-danger" : "text-muted hover:bg-sunken hover:text-ink"
      }`}
    >
      {children}
    </button>
